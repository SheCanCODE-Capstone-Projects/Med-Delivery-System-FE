  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        setPharmacy((p) => ({ ...p, latitude, longitude }));
        try {
          ...
          if (res.ok) {
            ...
            if (data.display_name) {
              setPharmacy((p) => ({ ...p, address: data.display_name }));
              setErrors((e) => ({ ...e, address: undefined }));
            }
          }
        } catch {
          // Reverse geocoding failed — coordinates are still saved, user can type address manually
        } finally {
          setGpsLoading(false);
        }
      },
